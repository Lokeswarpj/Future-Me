...
app.post("/generate-futureme", async (req, res) => {
  try {
...
Avoid all generic platitudes and cliches. Make every field highly personalized, emotionally resonant, and extremely tactical.`;
    ...
    console.error("Error in /api/generate-futureme (Firebase):", error);
...
app.post("/chat-futureme", async (req, res) => {
  try {
...
Reply in 2-5 short paragraphs. Give at least one clear action.`;
    ...
    console.error("Error in /api/chat-futureme (Firebase):", error);
    return res.status(500).json({
      success: false,
      error: "FutureMe could not respond right now. Try again."
    });
...
// Expose the Express app as exports.api
